...
async def quest_30(callback: types.CallbackQuery, state: FSMContext):
    ...
    async with SessionLocal() as session:
        ...
        steps = [
            {
                "text": "Привет! На связи КОМАНДА ЗАПУСКА! Я расскажу тебе, как ловко забирать деньги из карманов наших клиентов и при этом не угодить за решетку!",
                "button": "Круто!"
            },
            {
...
async def quest_32(callback: types.CallbackQuery, state: FSMContext):
    ...
    msg = await callback.message.answer_video(video=video,caption=
        "Квест 32: Ценности компании\nТут к тебе генеральный директор заглянул 😱\nИзучи ценности и пройди тест\n\n"
