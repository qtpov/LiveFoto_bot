...
async def quest_21(callback: types.CallbackQuery, state: FSMContext):
    ...
    message = await callback.message.answer(
        ...
        "Чтобы пройти квест 21, просто посмотри по сторонам и сосчитай всех коллег, кто сегодня с тобой на смене.\т"
        "Введи точное число ! 🧑‍🤝‍🧑👀"
    )
...
async def finish_quest26(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Когда будешь готов(а) — возвращайся и продолжай прокачивать скилл в разделе *«Квесты»*. Увидимся скоро! 💪📸",
